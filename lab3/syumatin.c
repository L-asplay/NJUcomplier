...
void init_func_read(){
   HashNodeFunc func_read=malloc(sizeof(struct HashNodeFunc_ ));
   func_read->name="read";
   func_read->retyp=malloc(sizeof(struct Type_));
   func_read->retyp->kind=BASIC;
   func_read->retyp->basic=0;
   func_read->argvs=malloc(sizeof(struct FieldList_ ));
   func_read->argvs->tail = NULL;
   func_read->tail = NULL;
   InsertFunc(func_read);
}

void init_func_write(){
   HashNodeFunc func_write=malloc(sizeof(struct HashNodeFunc_ ));
   
   func_write->name="write";

   func_write->retyp=malloc(sizeof(struct Type_));
   func_write->retyp->kind=BASIC;
   func_write->retyp->basic=0;

   func_write->argvs=malloc(sizeof(struct FieldList_ ));
   func_write->argvs->tail = malloc(sizeof(struct FieldList_ ));
   func_write->argvs->tail->type=malloc(sizeof(struct Type_));
   func_write->argvs->tail->type->kind=BASIC;
   func_write->argvs->tail->type->basic=0;
   func_write->argvs->tail->tail=NULL;

   func_write->tail = NULL;
...
Type Exp(Node* r) {
    ...
    //| ID LP RP
       ...
       if (resFunc != NULL) {
         ...
         Args(NodeArgs, resFunc->argvs->tail, resFunc->name, NodeId->lnumber);
...
void Args(Node* r, FieldList arg, char* name, int l) {
